...
export default function Dashboard3() {
  ...
  const dbRef = ref(getDatabase());
    get(child(dbRef, "History/workerhistorylastid")).then((snapshot) => {
    if (snapshot.exists()) {
  ...
  function Fetch()
  {
    ...
    get(child(db, `registered_worker/${userid}`)).then((snapshot) => {
      if (snapshot.exists()) {
          ...
          <Card sx={{ maxWidth:10}}>
            <CardActionArea>
              <CardMedia
                ...
                height="10"
                width="5"
                image="https://i.postimg.cc/L6yCdz8k/card.jpg"
                alt="green iguana"
              />
              <CardContent>
